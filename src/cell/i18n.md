...
Whatever the chrome renders comes out of one resolution chain — the active locale picks a dictionary out of the registry, and the overlay for that locale is deep-merged on top:

<DiagramFlow vertical label="Locale resolution: the active locale selects a dictionary from the registry, an overlay is merged on top, and the result is i18n.strings" :steps="[
  { label: 'Active locale', note: 'MountOptions.locale, then i18n.setLocale(name)' },
  { label: 'Dictionary registry', note: 'bundled en / ja, plus i18n.register(name, dict)' },
  { label: 'Overlay for that locale', note: 'MountOptions.strings, then i18n.extend(name, partial)' },
  { label: 'i18n.strings', note: 'what built-in chrome and extensions read' }
]" />
...
<DiagramFlow label="The locale round trip: setLocale emits localeChange, the host stores the value, and the next mount passes it back as MountOptions.locale" steps="i18n.setLocale(name) → localeChange event → host writes storage → next mount: locale option" />
