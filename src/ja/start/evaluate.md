...
::: warning Python の未対応
`evaluateFormulaText` / `evaluateConditionalFormula` は C API、Node アドオン、WASM でのみ利用できます。Python バインディングにはまだ相当する機能がありません。Python でワークブックの文脈が必要な場合は、`formulon.eval_formula` で使い捨ての数式を評価するか、`set_formula` でセルに書き込んで `recalc()` を呼んでください。
:::
