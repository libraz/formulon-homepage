...
下のパネルは、同じ手順をブラウザ内で実行します。ワークブックを選ぶ（またはサンプルを生成する）と、バイト列を `loadBytes()` で読み込み、再計算し、セルを一覧して、`.xlsx` のバイト列として書き出したものをダウンロードできます。表に並ぶ値は `recalc()` の後に読み取ったものなので、ファイルに保存されていた値ではなくエンジンが計算した値です。
