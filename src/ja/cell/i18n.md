...
::: tip UI ロケールと engine の互換性プロファイルは別
engine 側のロケール挙動（日付パース・通貨・リスト区切り）は Excel の **compatibility profile** で決まり、UI の `i18n.setLocale` とは独立です。UI をフランス語にしても数式評価結果は変わりません。engine 側は [ロケールプロファイル](/ja/compatibility/locale-profiles) を参照。
:::
...
- [埋め込みガイド](/ja/cell/embedding) ─ preset と extension の組み合わせ
- [API 一覧](/ja/cell/api) ─ events / store / command helper
