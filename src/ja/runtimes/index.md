...
| [Native Node](/ja/runtimes/node-native) | プラットフォーム別の `.node` addon を配置できる Node service |
...
flowchart LR
  ...
  NN --> NODE[Node service<br/>native 配置可]
...
| API ページ | 目的 |
| --- | --- |
| [API 一覧](/ja/api/surfaces) | パッケージごとの役割と host 側の責務を比較 |
...
実行環境の選択だけでは Excel 互換性は保証できません。重要なワークブックで使う前に [互換性](/ja/compatibility/) を確認し、自分のファイルで fixture を実行してください。
